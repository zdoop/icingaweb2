...
class CatalogParser
{
    ...
    public function parse()
    {
        ...
        while ($this->checkStack()) {
            ...
            if (! isset($currentEntry['obsolete'])) {
     ...
     * @param   array   $returnedValue  The value returned by handleStack
    ...
    protected function processParsedValues($currentEntry, $returnedValue, $lastType, $lastNumber)
    {
        ...
        $returnedValue = $this->resolveEscapedChars($returnedValue);
        if ($lastType === 'msgstr') {
     ...
     *
     * @param   string  $string     The string to resolve escaped characters in
     *
     * @return  string
     */
    protected function resolveEscapedChars($string)
    {
        foreach ($this->escapedChars as $key => $value) {
